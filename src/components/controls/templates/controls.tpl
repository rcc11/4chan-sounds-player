`<div class="${ns}-controls ${ns}-row ${data.inline ? `${ns}-inline-controls` : '' }" data-audio="${data.audioId}">
	<div class="${ns}-col-auto">
		...
		${data.inline && !data.multiple ? '' : `
			<div class="${ns}-media-control ${ns}-next-button ${ns}-hover-fill" @click='${data.actions.next}' data-hide-id="next">
				${Icons.skipEnd} ${Icons.skipEndFill}
			</div>
		`}
	</div>
	<div class="${ns}-col" data-hide-id="seek-bar">
		<div class="${ns}-seek-bar ${ns}-progress-bar" @pointdrag='${data.actions.seek}'>
	...
	<div class="${ns}-col-auto" data-hide-id="volume">
		...
		<div class="${ns}-volume-bar ${ns}-progress-bar" @pointdrag='${data.actions.volume}' data-hide-id="volume-bar">
			<div class="${ns}-full-bar">
