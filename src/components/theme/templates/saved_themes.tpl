`<div class="${ns}-row ${ns}-saved-themes">
	...
	${Player.config.savedThemesOrder.map((name, i) => `
		<div class="${ns}-row ${ns}-align-start ${ns}-sub-settings" data-theme="${name}" style="order: ${i}">
			...
			<div class="${ns}-col">
				<a href="#" class="${ns}-heading-action ${ns}-apply-theme" data-handler="theme._apply" data-property="savedTheme">${Player.config.selectedTheme === name ? Icons.checkSquare : Icons.square}</a>
